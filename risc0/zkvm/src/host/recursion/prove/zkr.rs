...
use risc0_circuit_recursion::{
    control_id::{
        BLAKE2B_CONTROL_IDS, BLAKE3_CONTROL_IDS, BN254_IDENTITY_CONTROL_ID,
...
fn get_zkr(name: &str, hashfn: &str) -> Result<(Program, Digest)> {
    let control_ids: &[(&str, Digest)] = match hashfn {
        ...
        // [blake3] ⭐ NOW THE FULL 32-PROGRAM TABLE. This arm carried `identity.zkr` alone
        // until the generator could emit all of them, exactly as the blake2b arm above did.
        // CROSS-CHECK: the retired one-entry stub had been computed by a separate probe, and its
        // identity.zkr digest 80545be5..a94d is bit-identical to the generated table's — two
        // independent derivations of the same control ID.
