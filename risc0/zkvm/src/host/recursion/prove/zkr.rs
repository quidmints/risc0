...
fn get_zkr(name: &str, hashfn: &str) -> Result<(Program, Digest)> {
    let control_ids: &[(&str, Digest)] = match hashfn {
        ...
        // [blake3] NO TABLE YET, AND THE ORDER IS FORCED RATHER THAN CHOSEN: a control
        // ID is `Program::compute_control_id(suite)`, so the SUITE MUST COMPILE BEFORE
        // ANY DIGEST EXISTS. Adding an empty or placeholder table here would fail with
        // "failed to find lift_rv32im_v2_N.zkr in the list of control IDs", which reads
        // like a missing program rather than an ungenerated table. So blake3 falls to
        // the bail below until the generator has run against this suite.
